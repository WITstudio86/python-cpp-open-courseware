# 第 46 节 · 项目 13：打字速度测试（下）

## 学习目标
- 学会用 JSON 保存历史打字成绩 📝
- 学会用 Listbox 做排行榜 🏆
- 学会用 Radiobutton 实现难度选择（短文/中文/长文）🎚️

## 本节课成品
完整的打字测试工具！能选难度、计时打分、保存历史成绩、查看排行榜。
...
def save_score(wpm, accuracy, difficulty):
    """保存成绩到历史记录"""
    record = {
...
def show_leaderboard():
    ...
    if os.path.exists(HISTORY_FILE):
        ...
        for i, record in enumerate(history, 1):
            ...
            line = f"{medal} {record['wpm']:6.1f} 字/分  |  {record['accuracy']}%  |  {record['difficulty']}  |  {record['date']}"
...
SAMPLES_HARD = [
    "千里之行，始于足下。学习编程也是一步一步来的，不能着急，要打好基础。",
    "学习编程就像搭积木，一块一块地累积起来，最终搭建出属于自己的程序世界。",
]

difficulty = tk.StringVar(value="medium")  # 默认中等

tk.Radiobutton(root, text="🌱 短文", variable=difficulty,
               value="easy", bg="#f5f5f5", font=("微软雅黑", 11)).pack(side=tk.LEFT, padx=5)
tk.Radiobutton(root, text="🌿 中文", variable=difficulty,
               value="medium", bg="#f5f5f5", font=("微软雅黑", 11)).pack(side=tk.LEFT, padx=5)
tk.Radiobutton(root, text="🌳 长文", variable=difficulty,
...
tk.Label(diff_frame, text="难度：", bg="#f5f5f5", font=("微软雅黑", 11)).pack(side=tk.LEFT)
...
start_btn = tk.Button(btn_frame, text="▶️ 开始测试", command=lambda: start_test(),
...
1. **选"短文"难度**，测一次，看成绩
2. **选"长文"难度**，再测一次，对比两次的 WPM
3. **打开排行榜**，看看前三名是哪几次成绩
4. **打开 typing_history.json** 看看保存的数据格式
5. **挑战**：增加一个"清除历史记录"功能
6. **挑战**：在排行榜中增加"按准确率排序"或"按难度筛选"功能

---

## 小贴士
- `StringVar` 是 tkinter 的特殊变量，用来关联 Radiobutton 的选中值
- `datetime.now().strftime("%Y-%m-%d %H:%M")` 格式化当前时间为字符串
- `history.sort(key=lambda x: x["wpm"], reverse=True)` 按 WPM 降序排列
- `history[:20]` 使用列表切片只保留前 20 条记录
- Radiobutton 的 `value` 属性定义了选中时 `StringVar` 的值
- 排行榜用 `["🥇", "🥈", "🥉"][i-1]` 给前三名加奖牌 emoji
